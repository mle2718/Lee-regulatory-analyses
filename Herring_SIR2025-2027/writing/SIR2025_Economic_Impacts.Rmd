...
Fishery conditions in 2023 are used as a baseline.  In 2023, the ACL was 13,287mt and landings were 10,038mt.  The specifications recommended by the Herring Committee for are 2,710 mt, 6,854 mt, and 6,854 mt in 2025-2027 respectively.  The ABC control rule would set ACLs of 2,710 mt, 6,854 mt, and 11,404 mt in 2025-2027 respectively.  Recent catches have been close to the ACLs, and therefore, this analysis assumes that catch is equal to the ACLs in each of the specification years. This assumption seems particularly reasonable since both the proposed ACLs and those implied by the ABC control rule are well below recent values.
...
\mbox{Projected Revenue} &=  (889 - 6.002*\mbox{landings}) *\mbox{landings}\label{eq:predicted_landings}
\end{align}
...
The 2023 actual landings of  **`r baseline2$Landings`**mt is used as the baseline. The price model projects prices of  **`r baseline2$Price` per metric ton** and real gross revenues derived from Atlantic herring of **`r baseline2$Revenue`** under baseline conditions. Both of these figures are higher than the actual prices (**`r baseline2$Actual_Price` per metric ton**) and revenues (**`r baseline2$Actual_Revenue`**). On average, predictions from the price model will match the actual prices, but in any particular year, they will be different. Nevertheless, we use the projected prices in the construction of baseline revenue so that an model errors are removed when comparing effects of different sets of ACLs to the baseline.  

#### Benefits (Gross Revenues) of the draft action

Table \ref{tab:prices_revenues_table} summarizes the annual prices, revenues, and changes of the draft action relative to the baseline. The draft action will result substantially lower landings relative to the baseline. The decrease in landings results in modestly higher Atlantic herring prices. Overall, the model projects a **decrease in revenues of `r proj_rev2$rc`** over the 2025-2027 time period. When discounted at a 2% discount rate, this corresponds to a net present value **decrease of `r  proj_rev2$rev2`**.
...
Relative to the draft action, the specifications associated with the ABC control rule have higher landings and revenues in 2027.  Because the Atlantic herring ACL is higher in 2027, this is less likely to rebuild the overfished Atlantic herring stock, however we do not attempt to monetize this probability.
