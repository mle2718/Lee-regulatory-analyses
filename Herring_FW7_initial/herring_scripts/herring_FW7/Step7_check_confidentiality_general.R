...
# A8 LD area
Amend8_LD = readOGR(dsn=file.path(ML.GIS.PATH,"herring_A8", "Final_A8_Localized_Depletion_Closure"), layer="Final_A8_Localized_Depletion_Closure", verbose=F)
Amend8_LD = spTransform(Amend8_LD, CRS=PROJ.USE)
...
rasterlist<- lapply(as.list(list.dirs(path=regime.geotiff.path, recursive=T)), list.files, recursive=F, full.names=T, pattern=tif_pat)
...
# Create a list of rasters that are in that GROUPUNITPATH 
  ...
  #rescale to "total pounds per square km"
  rescale_factor<-4
...
# Min-Yang's code changes for reclassification to 1-6 from value bins
  ...
  write.csv(myclass_matrix, file=file.path(confid.regime.geotiff.path,cutnames),  row.names=FALSE)
...
# Now set up to plot this GROUP of rasters, with the above-determined binning scheme
  ...
  rawPNG.file <- file.path(confid.regime.image.path, png_filename)
  
  
  my.regime<-temp_R_list$REGIME[t]
  
  my.regime[which(my.regime==1)]<-"2008-2013"
  my.regime[which(my.regime==2)]<-"2014-2018"
  ...
  # Filename for RECLASSIFIED plotted map png
      ...
      reclassPNG.file <- file.path(confid.regime.image.path, reclass_filename)
...
# OUTPUT  for the RAW raster 
      ...
      print(jcons+ layer(sp.polygons(my_basemap2, fill=mylandfill))
            
            + layer(sp.polygons(my_HMAs)) + layer(sp.polygons(Amend8_LD)) 
...
##### 
      ...
      writeRaster(myr2, file.path(confid.regime.geotiff.path,reclass_gtiff_name), format="GTiff", overwrite=T)
      ...
      print(rclassed + layer(sp.polygons(my_basemap2, fill=mylandfill))
            + layer(sp.polygons(my_HMAs)) + layer(sp.polygons(Amend8_LD)) 
...
############################
####
csv_out<-file.path(confid.regime.image.path,"parsed_groups.csv")
