...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"2016-03-11_rasterexport") #This has Predators yearly in it
...
sp2<-"81"
z2<-paste(sp2,"*.tif",sep="")
mypat2<-glob2rx(z2)
sp3<-"352"
z3<-paste(sp3,"*.tif",sep="")
mypat3<-glob2rx(z3)
...
for (i in 1:nrow(parsed_list1)) {
  ...
  for (square in i_care)
  {
    ...
    my_year1[[length(my_year1)+1]] = parsed_list1$YEAR[i]
...
my_year2=list()
my_block2=list() #A place to hold things
my_block_qty2=list() #A place to hold things
my_annual_qty2=list() #A place to hold things

for (i in 1:nrow(parsed_list2)) {
  myras2<-raster(parsed_list2$FILEPATH[i])
  mya<-cellStats(myras2,'sum')
  ...
  for (square in i_care)
  {
    ...
    sub<-my_basemap[my_basemap@data$NERO_ID == square & !is.na(my_basemap@data$NERO_ID) , ]
    rvals<-extract(myras2,sub)
    tsub<-unlist(lapply(rvals, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
    my_block_qty2[[length(my_block_qty2)+1]] = tsub
    my_annual_qty2[[length(my_annual_qty2)+1]] = mya
    my_block2[[length(my_block2)+1]] = square
    my_year2[[length(my_year2)+1]] = parsed_list2$YEAR[i]
...
my_year3=list()
my_block3=list() #A place to hold things
my_block_qty3=list() #A place to hold things
my_annual_qty3=list() #A place to hold things

for (i in 1:nrow(parsed_list3)) {
  myras3<-raster(parsed_list3$FILEPATH[i])
  mya<-cellStats(myras3,'sum')
  ...
  for (square in i_care)
  {
    ...
    sub<-my_basemap[my_basemap@data$NERO_ID == square & !is.na(my_basemap@data$NERO_ID) , ]
    rvals<-extract(myras3,sub)
    tsub<-unlist(lapply(rvals, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
    my_block_qty3[[length(my_block_qty3)+1]] = tsub
    my_annual_qty3[[length(my_annual_qty3)+1]] = mya
    my_block3[[length(my_block3)+1]] = square
    my_year3[[length(my_year3)+1]] = parsed_list3$YEAR[i]
...
y1<-do.call(rbind, lapply(my_year1, data.frame, stringsAsFactors=FALSE))
y2<-do.call(rbind, lapply(my_year2, data.frame, stringsAsFactors=FALSE))
y3<-do.call(rbind, lapply(my_year3, data.frame, stringsAsFactors=FALSE))


b1<-do.call(rbind, lapply(my_block1, data.frame, stringsAsFactors=FALSE))
b2<-do.call(rbind, lapply(my_block2, data.frame, stringsAsFactors=FALSE))
b3<-do.call(rbind, lapply(my_block3, data.frame, stringsAsFactors=FALSE))
...
blockq2<-do.call(rbind, lapply(my_block_qty2, data.frame, stringsAsFactors=FALSE))
annualq2<-do.call(rbind, lapply(my_annual_qty2, data.frame, stringsAsFactors=FALSE))

blockq3<-do.call(rbind, lapply(my_block_qty3, data.frame, stringsAsFactors=FALSE))
annualq3<-do.call(rbind, lapply(my_annual_qty3, data.frame, stringsAsFactors=FALSE))


my_output1<-data.frame(y1,b1,blockq1, annualq1)
my_output2<-data.frame(y2,b2,blockq2, annualq2)
my_output3<-data.frame(y3,b3,blockq3, annualq3)


write.table(my_output1,(file.path(YOUR.STORAGE.PATH, paste(sp1,".csv", sep=""))), sep=",", row.names=FALSE, col.names=c("year", "block", "quantity", "aggregate q"))  

write.table(my_output2,(file.path(YOUR.STORAGE.PATH, paste(sp2,".csv", sep=""))), sep=",", row.names=FALSE, col.names=c("year", "block", "quantity", "aggregate q"))  

write.table(my_output3,(file.path(YOUR.STORAGE.PATH, paste(sp3,".csv", sep=""))), sep=",", row.names=FALSE, col.names=c("year", "block", "quantity", "aggregate q"))  
