...
#filelist = lapply(as.list(list.dirs(path=bonusfolder, recursive=F)), list.files, recursive=T, full.names=T, pattern="*.gri")
...
#### (2quick) instead of doing the filelist, use geret's pre-constructed RasterDate.Rdata file 
...
for (yr in START.YEAR:END.YEAR)  {
  ...
  FINALa=FINALa[which(FINALa$NESPP3 %in% c("168")),]
  FINALa=FINALa[which(FINALa$GEARCODE %in% c("OTM","PTM")),] # <-MWT and PUR
  FINALa=FINALa[which(FINALa$QTYKEPT > 0),]
...
FINAL$MY_GEAR[FINAL$GEARCODE=="OTF"]<-"OTF"
...
FINAL$MY_MARGIN<-paste(FINAL$MY_GEAR,FINAL$MONTH, sep="_")
