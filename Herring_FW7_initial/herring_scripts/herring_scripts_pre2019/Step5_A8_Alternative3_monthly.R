...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"HERRING_A8_09162016") #This has monthly MWT and PUR yearly in it
...
results=matrix(0,nrow=nrow(parsed_list1), ncol=9)
#NOT IN 1A
...
for (myr in 1:nrow(parsed_list1)){
    ...
    QNA<-cellStats(raster_subset,'sum')

    #Mask the values OUTSIDE the 6nm buffer to NA
    
    raster_subset_temp<-mask(raster_subset,my_50nm)
    Q50<-cellStats(raster_subset_temp,'sum')
    
    
    
    raster_subset_temp<-mask(raster_subset,my_35nm)
    Q35<-cellStats(raster_subset_temp,'sum')
    
    raster_subset_temp<-mask(raster_subset,my_25nm)
    Q25<-cellStats(raster_subset_temp,'sum')
    
    raster_subset_temp<-mask(raster_subset_temp,my_12nm)
    Q12<-cellStats(raster_subset_temp,'sum')
    
    raster_subset_temp<-mask(raster_subset_temp,my_6nm)
    Q6<-cellStats(raster_subset_temp,'sum')

    results[myr,1]<-parsed_list1$MONTH[myr]
    results[myr,2]<-quantity_grand_sum/(2204)
    results[myr,3]<-QNA/(2204)
    results[myr,4]<-Q6 /(2204)
    results[myr,5]<-Q12/(2204)
    results[myr,6]<-Q35/(2204)
    results[myr,7]<-Q25/(2204)
    results[myr,8]<-Q50/(2204)
    results[myr,9]<-parsed_list1$NAME[myr]
    ...
    table_name<-"alt34_results_monthly_fixed"
    write.table(results,(file.path(YOUR.STORAGE.PATH, paste0(table_name,".csv"))) , sep=",", row.names=FALSE, col.names=c("month", "grand total mt", "Areas1B23_mt", "In6_mt", "In12_mt", "In35_mt", "In25_mt", "In50_mt", "Name"))  
