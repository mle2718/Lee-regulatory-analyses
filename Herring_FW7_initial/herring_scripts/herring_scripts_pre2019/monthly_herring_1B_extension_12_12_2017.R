...
## OUTPUT LOCATIONS
#####################################################
exportfolder = file.path(YOUR.PROJECT.PATH,"New1B_option")
...
my_HMA_extension = readOGR(dsn=file.path(YOUR.PROJECT.PATH, "New1B_option"), layer="hma_1b_extension_only", verbose=F)
my_HMA_extension = spTransform(my_HMA_extension, CRS=PROJ.USE)

my_HMA_mod = readOGR(dsn=file.path(YOUR.PROJECT.PATH, "New1B_option"), layer="hma_1b_proposed", verbose=F)
my_HMA_mod = spTransform(my_HMA_mod, CRS=PROJ.USE)


# foreach buffer file:  Intersect the buffer with the HMA. Store this somewhere  Save this as AREA_SHP.
my_6nm<-my_HMA_extension
my_12nm<-my_HMA_mod
...
parsed_list1a$MONTH = sub('_$',' ',str_sub(parsed_list1a$NAME,1,2))
...
for (i in 1:nrow(parsed_list1a)) {
  ...
  mya<-cellStats(myras,'sum')
    ...
    rvals12<-extract(myras,my_12nm) #Subset raster data within 6nm
    r12<-unlist(lapply(rvals12, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
...
my_output1<-data.frame(y1,name,inside6, inside12, annualq1)

write.table(my_output1,(file.path(YOUR.STORAGE.PATH, "hma1b_changes.csv")), sep=",", row.names=FALSE, col.names=c("month", "name", "in1B_extension", "in1b_mod", "aggregate q"))  
