...
#Initialize a 4x12 matrix to store the data
# Month, quantity_grand_sum, q114, q114inside
...
for (myr in 1:nrow(parsed_list1)){
  ...
  results[myr,2]<-quantity_grand_sum/(2204*6)
  results[myr,3]<-QNA/(2204*6)
  results[myr,4]<-Q6 /(2204*6)
...
table_name<-"alt1_results_monthly"
write.table(results,(file.path(YOUR.STORAGE.PATH, paste0(table_name,".csv"))) , sep=",", row.names=FALSE, col.names=c("month", "grand total mt", "cell114", "Inside6_mt", "Name"))  
