...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"2016-07-26_rasterexport") #This has monthly MWT and PUR yearly in it
...
## OUTPUT LOCATIONS
#####################################################
exportfolder = file.path(YOUR.PROJECT.PATH,"A8_Alternatives34")
...
for (myc in 1:nrow(parsed_list1))  {
...
#after this, you will have 1 raster in memory,   
## YOU"VE GOTTEN TO HERE. TO Finish this off, you need to read in the HERRING MANAGEMENT AREA. YOU NEED TO DROP OUT AREA 1A.  
#Keep and dissolve 1B, 3, and 2.  
#Mask the aggregate raster to remove the things outside of 1B, 3, and 2.
#Apply the 12nm buffer
#Apply the 35nm buffer

#This has ALL landings by MWT fleet everywhere, year round.
myras<-aggregate
...
quantity_grand_sum<-cellStats(myras,'sum')
    ...
    #This subset now just contains landings by MWT fleet that are NOT in 1A.
    
    raster_subset<-mask(myras,sub)
    quantity_not_1A<-cellStats(raster_subset,'sum')
    
    #Mask the values OUTSIDE the 6nm buffer to NA
    
    raster_subset_temp<-mask(raster_subset,my_6nm)
    
    quantity_not_1A_inside6<-cellStats(raster_subset_temp,'sum')
    
    raster_subset_temp<-mask(raster_subset,my_12nm)
    
    quantity_not_1A_inside12<-cellStats(raster_subset_temp,'sum')
    
    
    raster_subset_temp<-mask(raster_subset,my_35nm)
    
    quantity_not_1A_inside35<-cellStats(raster_subset_temp,'sum')
    
    
    
    
    table_name<-alt34_results
    results<-rbind(cbind(quantity_grand_sum, quantity_not_1A,quantity_not_1A_inside6), cbind(quantity_grand_sum, quantity_not_1A,quantity_not_1A_inside12), cbind(quantity_grand_sum, quantity_not_1A,quantity_not_1A_inside35))
    results<-results/2204
    results<-cbind(rbind(6,12,35),results)
    write.table(results,(file.path(YOUR.STORAGE.PATH, paste0(table_name,".csv"))) , sep=",", row.names=FALSE, col.names=c("buffer", "grand total", "Areas1B23", "Inside Buffer"))  
  ...
  #  HTML(paste0("START YEAR: ", START.YEAR), w, T)
  #  HTML(paste0("END YEAR: ", END.YEAR), w, T)
    
    #buffer=6 means 6nm buffer.
    #buffer=35 means 35nm buffer 
    #Grand total is all herring qtykept by mwt
    #Areas1b23 is landings in areas 1b, 2, 3 by MWT
    #inside buffer is the amount of landings inside the buffer 
    
    #Bonus, this also answers the Area 1A alternative!
