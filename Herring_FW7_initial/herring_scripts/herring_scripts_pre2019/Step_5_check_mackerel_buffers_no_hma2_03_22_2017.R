...
AREA.PATH= exportfolder  #To recap, we are storing things in the folder "exportfolder" which is in the directory defined by YOUR.DATA.PATH
#################################################################################
# END OF SECTION 1  
#################################################################################
...
AREA_SHP50<-gIntersection(sub1B3,my_50nm)
...
#setup the results matrix -- IDNUM, 6,12,25, 35, and 50 percentages
 ...
 for (yr in START.YEAR:END.YEAR)  {
   ...
   for (ar in MGAREA){
     ...
     save(ACTIVITY, file = paste(AREA.PATH, "/",AREA,"_mackerel_no2_",YEAR,".RData", sep = ''))
   }
