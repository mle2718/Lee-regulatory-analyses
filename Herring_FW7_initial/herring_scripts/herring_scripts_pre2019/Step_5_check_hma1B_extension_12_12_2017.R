...
AREA_SHP50<-my_HMA_mod
 AREA_SHP35<-my_HMA_extension
 mybuffers<-c(50,35)
 ...
 for (yr in START.YEAR:END.YEAR)  {
   ...
   for (ar in MGAREA){
     ...
     AREA = ar
          ...
          sfExport("AREA","AREA_SHP50","AREA_SHP35")
     ...
     ACTIVITY = sfLapply(fileswanted,function(x,...) {     #usually run in sfLapply
       ...
       id.list <- matrix(nrow = NR,ncol = 5)
       colnames(id.list) <- c("IDNUM","Area","Year","Inside50", "Inside35")
