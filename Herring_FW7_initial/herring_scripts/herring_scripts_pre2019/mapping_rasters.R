...
# March 11, 2016
# version 0.2
# Changes: Fixed handling of the upper and lower bounds of Jenks breaks. 
#################################################################################
...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"2016-03-07_rasterexport") 
...
# I WANT TO Separately MAP ALL THE GEOTIFS THAT MATCH "MWT_168 and PUR_168 
mypat1<-glob2rx("MWT_168*.tif")
mypat2<-glob2rx("PUR_168*.tif")
...
for (i in 1:nrow(parsed_list1)) {
  ...
  #To fix this, we will set all raster values greater than
  ...
  #This runs levelplot with default binning
  p<-levelplot(myras, margin=FALSE, main=title_opts1, par.setting=mycoloropts,scales=myscaleopts)

  #This runs levelplot with breaks defined by "mybreaks_class1" (JENKS BREAKS) above.  
