...
exportfolder = file.path(YOUR.PROJECT.PATH,paste0("confid_check_2017-10-13"))
   ...
   for (ar in MGAREA){
     ...
     ACTIVITY = sfLapply(fileswanted,function(x,...) {     #usually run in sfLapply
       ...
       for (i in 1:(NROW(BR)-1)){ #use row # of what's in that "brick" 
         ...
         for (j in t.index) {   
           tt = raster(x[[j]])
           tt = mask(tt, council_pa, byid=c(T,F), id=row.names(tt))
     ...
     save(ACTIVITY, file = paste(YOUR.STORAGE.PATH, "/",AREA,YEAR,".RData", sep = ''))
     
     #This line is not tested to work
     write.dta(ACTIVITY, file.path(YOUR.STORAGE.PATH,paste0(AREA,YEAR,".dta"))) 
