...
## OUTPUT LOCATIONS
#####################################################
exportfolder = file.path(YOUR.PROJECT.PATH,paste0("A8_background_2017-03-16"))
...
baseraster <-raster(file.path(GD.RASTERS,"Data","BASERASTER_AEA_2.grd"))
...
parsed_list1 = do.call(rbind, lapply(list1, function(xx) {
  ...
  names(xx) = "FILENAME" 
  return(xx) }) )                                         
parsed_list1$first_part =  sapply(parsed_list1$FILENAME, USE.NAMES=F, function(zz) {
...
parsed_list1$YEAR <- as.numeric(substrRight(parsed_list1$NAME,4))
...
parsed_list1$GEAR = sapply(parsed_list1$NAME, USE.NAMES=F, function(zz) {
  temp = do.call(rbind,strsplit(as.character(zz), split = "_"))
  return(temp[1]) })
...
parsed_list1$REGIME[which(parsed_list1$YEAR>=2001 & parsed_list1$YEAR<=2007)]<-1
parsed_list1$REGIME[which(parsed_list1$YEAR>=2008 & parsed_list1$YEAR<=2015)]<-2
parsed_list1$REGIME[which(parsed_list1$REGIME==1 & parsed_list1$YEAR>=2007 & parsed_list1$MONTH>=6)]<-2


gearlist <- c("MWT", "BOT","PUR")

for (myg in gearlist){ 
  for (mymonth in 1:12) {
    
for (myr in 1:2)  {
   yearfileswanted <- parsed_list1[which(parsed_list1$REGIME==myr & parsed_list1$MONTH==mymonth & parsed_list1$GEAR==myg),]
   outfilename<-paste0("herring_pounds_",myg,"_month_",mymonth,"_regime_",myr,".tif")
...
# You've coded this to do just BOT.  You should code this to loop through the distinct types of gear. This would involve 1 more loop.
# You also need to modify the storage folder.
