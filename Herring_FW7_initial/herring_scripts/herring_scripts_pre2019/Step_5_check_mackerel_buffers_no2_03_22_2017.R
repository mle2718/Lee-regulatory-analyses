...
sub<-sub[sub@data$GARFO_ID != c("G000169"), ]
sub<-gBuffer(sub,byid=FALSE, width=0.001)
#Load in the HMA
...
AREA_SHP50<-gIntersection(sub,my_50nm)
...
#setup the results matrix -- IDNUM, 6,12,25, 35, and 50 percentages
 ...
 for (yr in START.YEAR:END.YEAR)  {
   ...
   for (ar in MGAREA){
     ...
     save(ACTIVITY, file = paste(AREA.PATH, "/",AREA,"_mackerel_",YEAR,".RData", sep = ''))
   }
