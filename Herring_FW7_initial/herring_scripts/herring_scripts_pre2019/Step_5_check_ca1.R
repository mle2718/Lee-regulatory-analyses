...
AREA_SHP<- CAI_grnd[CAI_grnd$id==2,]
...
for (yr in START.YEAR:END.YEAR)  {
  ...
  for (ar in MGAREA){
    ...
    ACTIVITY = sfLapply(fileswanted,function(x,...) {     #usually run in sfLapply
      ...
      colnames(id.list) <- c("IDNUM","Area","Year","Inside")
        ...
        for (i in 1:(NROW(BR)-1)){ #use row # of what's in that "brick" 
          ...
          for (j in t.index) {   
            tt = raster(x[[j]])
            tt = mask(tt, AREA_SHP[AREA_SHP$id==AREA,], byid=c(T,F), id=row.names(tt))
    ...
    save(ACTIVITY, file = paste(AREA.PATH, "/",AREA,YEAR,"check6",".RData", sep = ''))
  }
