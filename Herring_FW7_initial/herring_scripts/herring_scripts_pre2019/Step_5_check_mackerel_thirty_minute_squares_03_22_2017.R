...
AREA.PATH= exportfolder
 #To recap, we are storing things in the folder "exportfolder" which is in the directory defined by YOUR.DATA.PATH
#################################################################################
# END OF SECTION 1  
#################################################################################
...
for (yr in START.YEAR:END.YEAR)  {
  ...
  for (ar in MGAREA){
    ...
    save(ACTIVITY, file = paste(AREA.PATH, "/",AREA,"_",YEAR,"_mackerel_thirty_mins",".RData", sep = ''))
  }
