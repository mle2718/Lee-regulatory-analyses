...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"MACKEREL_ALL_GEARS_11172016") 
...
mypat1<-glob2rx("*mackerelQTYKEPT*.tif")
...
#THIS SETS UP A RASTER THEME THAT IS GOOD FOR COLOR BLIND PEOPLE
temp_dichrome<-dichromat(terrain.colors(numclasses))
#THIS SETS UP A RASTER THEME THAT IS GOOD FOR COLOR BLIND PEOPLE and includes a clear first bin.
...
parsed_list1a$MONTH = str_sub(parsed_list1a$NAME,1,2)
parsed_list1a$MONTH = gsub("_","",parsed_list1a$MONTH)
parsed_list1a$outname = gsub("mackerelQTYKEPT","mackerelQ",parsed_list1a$NAME)

parsed_list1a <- parsed_list1a[which(parsed_list1a$YEAR>= 2008),]
...
#For all months, loop over the 



for (mymonth in 1:12)  {
  
  parsed_list1 <- parsed_list1a[which(parsed_list1a$MONTH==mymonth),]
...
for (i in 1:nrow(parsed_list1)) {
 ...
 #Set up 2 file names pfilename to store the maps.  
  ...
  jconsfilename=paste(parsed_list1$outname[i],"jcons",sep="_")
  ...
  #Overlay things and save to pngs.
  ...
  print(jcons+ layer(sp.lines(caplus)) + layer(sp.lines(Closedgf_04_15)) + layer(sp.polygons(my_basemap2, fill=mylandfill)))
  dev.off()
