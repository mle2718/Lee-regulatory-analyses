...
max.nodes = 16 # Set to 12 when working on Server?
...
AREA_SHP<-my_HMAs[my_HMAs@data$GARFO_ID == c("G000170") , ]

MGAREA = AREA_SHP$GARFO_ID
...
for (yr in START.YEAR:END.YEAR)  {
  ...
  for (ar in MGAREA){
    ...
    ACTIVITY = sfLapply(fileswanted,function(x,...) {     #usually run in sfLapply
      ...
      colnames(id.list) <- c("IDNUM","Area","Year","Inside")
        ...
        for (i in 1:(NROW(BR)-1)){ #use row # of what's in that "brick" 
          ...
          for (j in t.index) {   
            tt = raster(x[[j]])
            tt = mask(tt, AREA_SHP[AREA_SHP$GARFO_ID==AREA,], byid=c(T,F), id=row.names(tt)) ###YOU NEED TO CHANGE THIS TOO!!!
    ...
    save(ACTIVITY, file = paste(AREA.PATH, "/",AREA,YEAR,"hmas.RData", sep = ''))
  }
