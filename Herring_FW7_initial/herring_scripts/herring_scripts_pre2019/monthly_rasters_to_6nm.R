...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"2016-03-11_rasterexport") #This has monthly MWT and PUR yearly in it
...
sp2<-"MWT_168"
z2<-paste0(sp2,"*9999.tif")
mypat2<-glob2rx(z2)
#mypat3<-glob2rx("352*.tif")
#mypat4<-glob2rx("121*.tif")
...
#get the 9th and 10th characters from NAME. Discard the Q for the single digit months
parsed_list1$MONTH = sub('Q$',' ',str_sub(parsed_list1$NAME,9,10))
...
parsed_list2$NAME = sapply(parsed_list2$first_part, USE.NAMES=F, function(zz) {
  temp = do.call(rbind,strsplit(as.character(zz), split = "/"))
  return(temp[NCOL(temp)]) })
parsed_list2$MONTH = sub('Q$',' ',str_sub(parsed_list2$NAME,9,10))
...
for (i in 1:nrow(parsed_list1)) {
  ...
  mya<-cellStats(myras,'sum')
    ...
    rvals<-extract(myras,my_6nm) #Subset raster data within 6nm
    tsub<-unlist(lapply(rvals, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
    
    rvalsa<-extract(myras,my_12nm) #Subset raster data within 12nm
    tsuba<-unlist(lapply(rvalsa, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
    my_block_qty1a[[length(my_block_qty1a)+1]] = tsuba
    ...
    rm(sub, rvals, tsub, rvalsa, tsuba)
  }
...
for (i in 1:nrow(parsed_list2)) {
  ...
  mya<-cellStats(myras2,'sum')
    ...
    rvals<-extract(myras2,my_6nm)
    tsub<-unlist(lapply(rvals, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
    
    
    rvalsa<-extract(myras2,my_12nm)
    tsuba<-unlist(lapply(rvalsa, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
    
    my_block_qty2a[[length(my_block_qty2a)+1]] = tsuba
    
    
    my_block_qty2[[length(my_block_qty2)+1]] = tsub
    my_annual_qty2[[length(my_annual_qty2)+1]] = mya
    my_month2[[length(my_month2)+1]] = parsed_list2$MONTH[i]
...
y2<-do.call(rbind, lapply(my_month2, data.frame, stringsAsFactors=FALSE))
...
blockq1a<-do.call(rbind, lapply(my_block_qty1a, data.frame, stringsAsFactors=FALSE))

blockq2a<-do.call(rbind, lapply(my_block_qty2a, data.frame, stringsAsFactors=FALSE))


my_output1<-data.frame(y1,blockq1, blockq1a, annualq1)
my_output2<-data.frame(y2,blockq2, blockq2a, annualq2)


write.table(my_output1,(file.path(YOUR.STORAGE.PATH, paste(sp1,"monthly_inshore.csv", sep=""))), sep=",", row.names=FALSE, col.names=c("month", "quantity6", "quantity12", "aggregate q"))  

write.table(my_output2,(file.path(YOUR.STORAGE.PATH, paste(sp2,"monthly_inshore.csv", sep=""))), sep=",", row.names=FALSE, col.names=c("month", "quantity", "quantity12", "aggregate q"))  
