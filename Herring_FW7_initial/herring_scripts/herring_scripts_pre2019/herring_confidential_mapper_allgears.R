...
# For the real deal! 
YOUR.DATA.PATH= file.path("/net/home2/mlee/RasterRequests/A8_Regimes2017-06-13/") 
YOUR.CODE.PATH= file.path("/net/home2/mlee/RasterRequests/herring_scripts/") 

# Where output files are saved
YOUR.STORAGE.PATH = file.path(paste0("/net/home2/mlee/RasterRequests/A8_background_",todaysdate))
...
tif_pat<-glob2rx("allgears_full*.tif")
...
# Parse out the names of each raster's file path into chunks
    ...
    parsed_groups$block =  sapply(parsed_groups$NAME, USE.NAMES=F, function(zz) { # We are referring to "REVENUE" versus "QTYKEPT here
      temp <- do.call(rbind,strsplit(as.character(zz), split = "_")) 
...
#
        ...
        #Read in the rasters and add them together.
          ...
          for (pp in 1:nrow(parsed_groups)){
            myras<-raster(file.path(parsed_groups$FILEPATH[1]))
            f2<-extent(myras)
          ...
          cutnames<-paste0("allgears_summer_scaleB.csv")
          
          cutnames<-paste0("allgears_scaleA.csv")
          ...
          for (pp in 1:nrow(parsed_groups)){
            myras<-extend(raster(file.path(parsed_groups$FILEPATH[pp])),f3,value=0)
            working_regime<-parsed_groups$REGIME[pp]
            myr2<-reclassify(myras,myclass_matrix, include.lowest=TRUE, right=TRUE)
            

            #do you need to subset FINAL based on Month?
                        
            source(file.path(YOUR.CODE.PATH,"herring_confidential_checker.R"))
            
            pdf_filename<-paste0("allgears_full_regime_",parsed_groups$REGIME[pp] ,".pdf")
