...
# MWT only. Herring and Mackerel. All HMAs.
todaysdate = as.Date(Sys.time())
todaysdate= "2018-10-13"
#################################################################################
# Section 1 -- Set up directories 
#################################################################################
#################################################################################
...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"HERRING_ALL_GEARS_09162016")
...
## OUTPUT LOCATIONS
#####################################################
exportfolder = file.path(YOUR.PROJECT.PATH,paste0("NEFMC_PREF",todaysdate))
...
savefilename<-"PA2only_herring_pounds_all_gears"
#################################################################################
# END OF SECTION 1  
#################################################################################
...
my_basemap2 = spTransform(my_basemap2, CRS=PROJ.USE)
 ...
 hma2<-my_HMAs[my_HMAs@data$GARFO_ID == c("G000169"), ]
...
#6nm buffer
 council_PA_2only = readOGR(dsn=file.path(ML.GIS.PATH,"herring_A8", "Final_A8_Localized_Depletion_Closure"), layer="temp", verbose=F)
council_PA_2only = spTransform(council_PA_2only, CRS=PROJ.USE)


#tt<-gBuffer(council_PA_2only, width=10)
#################################################
## END SECTION 4 
#################################################
...
parsed_list1a<- parsed_list1a[which(parsed_list1a$YEAR>=2015),]
...
for (i in 1:nrow(parsed_list1a)) {
  ...
  rvalsPA<-extract(myras,council_PA_2only) #Subset raster data within 6nm
  rvalsPA<-unlist(lapply(rvalsPA, function(x) if (!is.null(x)) sum(x, na.rm=TRUE) else NA ))
  
  my_block_PA[[length(my_block_PA)+1]] = rvalsPA
...
inPA<-do.call(rbind, lapply(my_block_PA, data.frame, stringsAsFactors=FALSE))
annualq1<-do.call(rbind, lapply(my_annual_qty1, data.frame, stringsAsFactors=FALSE))


my_output1<-data.frame(name,inPA, annualq1)
colnames(my_output1)<-c("name", "inPA", "annual")  
write.table(my_output1, file.path(YOUR.STORAGE.PATH, paste0(savefilename,"_",todaysdate,".csv")), sep=",", row.names=FALSE)
