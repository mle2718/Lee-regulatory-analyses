...
FIELD = c("fishing_hours") #Like "REVENUE","QTYKEPT", "QTYDISC", or "CATCH"
...
for (yr in START.YEAR:END.YEAR)  {
  ...
  FINALa=FINALa[which(FINALa$GEARCODE %in% c("OTF")),] # <-MWT and PUR
...
FINAL = FINAL[which(!is.na(FINAL[["QTYKEPT"]])),]
FINAL = FINAL[which(FINAL[["QTYKEPT"]]!=0),]

FINAL = FINAL[which(!is.na(FINAL[[FIELD]])),]
FINAL = FINAL[which(FINAL[[FIELD]]!=0),]

#BE CAREFUL WITH THIS LINE.  There may be multiple records per IDNUM if you are summing up "effort" or "trip/gearid" characteristics, like fishing_hours. 
#The next line will quickly drop out any duplicate GEARIDs that are in your dataset. But be careful about sort order.  You don't "need" to pay attention to that for the current application of extracting herring effort. 
#FINAL = FINAL[!duplicated(FINAL$IDNUM),]
