...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"MACKEREL_ALL_GEARS_11172016") #This has monthly MWT and PUR yearly in it
...
## OUTPUT LOCATIONS
#####################################################
exportfolder = file.path(YOUR.PROJECT.PATH,"thirty_minute_extractions_07_11_2017")
...
FINAL=FINAL[which(FINAL$GEARCODE %in% c("OTM","PTM","OTF","OTR","PUR")),] # <-MWT and PUR
...
sub_1B23<-gBuffer(sub,width=.1, byid=F)
 ...
 AREA_SHP<-AREA_SHP[which(AREA_SHP@data$NERO_ID %in% c(114)),]

# foreach buffer file:  Intersect the buffer with the HMA. Store this somewhere  Save this as AREA_SHP.
AREA_SHP50<-gIntersection(AREA_SHP,my_50nm)
 AREA_SHP25<-gIntersection(AREA_SHP,my_25nm)
 AREA_SHP12<-gIntersection(AREA_SHP,my_12nm)
 AREA_SHP6<-gIntersection(AREA_SHP,my_6nm)
 ...
 for (yr in START.YEAR:END.YEAR)  {
   ...
   for (ar in MGAREA){
     ...
     AREA = ar
          ...
          sfExport("AREA","AREA_SHP50","AREA_SHP25","AREA_SHP12","AREA_SHP6")
     ...
     ACTIVITY = sfLapply(fileswanted,function(x,...) {     #usually run in sfLapply
       ...
       id.list <- matrix(nrow = NR,ncol = 7)
       colnames(id.list) <- c("IDNUM","Area","Year","Inside50", "Inside25","Inside12","Inside6")
     ...
     save(ACTIVITY, file = paste(AREA.PATH, "/",AREA,"_",YEAR,"committee_mackerel_confid.RData", sep = ''))
   }
