 ...
 fl <- read.csv("/net/home2/mlee/RasterRequests/individual_payload2016-03-28.csv")
...
#     return(temp[(NCOL(temp)-1)]) })
  ...
  for (yr in START.YEAR:END.YEAR)  {
    ...
    FINALa <- read.dbf(file = paste0(SSBdrive,"/Data/Archive/ExportAll",yr,".dbf"))
    ...
    FINALa=FINALa[which(FINALa$NESPP3 %in% c("168", "212")),]
  ...
  FINAL$MY_MARGIN<-paste(FINAL$MY_GEAR,FINAL$NESPP3,FINAL$MONTH, sep="_")
