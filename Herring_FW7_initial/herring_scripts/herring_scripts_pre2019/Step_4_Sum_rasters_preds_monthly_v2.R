...
for (yr in START.YEAR:END.YEAR)  {
  ...
  FINALa=FINALa[which(FINALa$NESPP3 %in% c("81", "269", "352")),]
  

  FINAL=rbind(FINAL, FINALa)
}

FINAL$MY_MARGIN<-paste(FINAL$NESPP3,FINAL$MONTH, FINAL$YEAR,sep="_")
...
max.nodes = min(max.nodes,as.numeric(length(unique(RASTER_FILE[[paste0(MARG)]]))))
