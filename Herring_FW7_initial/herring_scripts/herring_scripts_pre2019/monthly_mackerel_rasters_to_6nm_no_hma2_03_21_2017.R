...
## OUTPUT LOCATIONS
#####################################################
exportfolder = file.path(YOUR.PROJECT.PATH,"thirty_minute_extractions_2017-10-23")
...
my_50nm<-gIntersection(sub1B3,my_50nm)
 my_25nm<-gIntersection(sub1B3,my_25nm)
 my_12nm<-gIntersection(sub1B3,my_12nm)
 my_6nm<-gIntersection(sub1B3,my_6nm)
...
write.table(my_output1,(file.path(YOUR.STORAGE.PATH, "monthly_extra_inshore_fixed_mackerel_nohma2.csv")), sep=",", row.names=FALSE, col.names=c("month", "name", "quantity6", "quantity12", "quantity25", "quantity50", "aggregate q"))  
