...
YOUR.DATA.PATH=file.path(YOUR.PROJECT.PATH,"A8_background_2017-03-16") 
...
ras_scale<-1
ras_scale<-4/2204 #this will scale to metric tons per km^2
...
distinct_regimes<-unique(parsed_list1a$GROUP)
for (mygroup in distinct_regimes)  {
...
mybreaks_class1<-c(0,myclass$brks)*ras_scale
...
for (i in 1:nrow(parsed_list1)) {
  ...
  myras<-raster(parsed_list1$FILEPATH[i])*ras_scale
 ...
 #Set up 2 file names pfilename to store the maps.  
  ...
  my.regime[which(my.regime==1)]<-"Pre-2007"
  my.regime[which(my.regime==2)]<-"Post-2007"
  
  my.month<-parsed_list1$MONTH[i]
  my.month[which(my.month==0)]<-"N/A"
  my.month<-paste0("Month=",my.month)

  my.gear<-paste0("Gear=",parsed_list1$GEAR[i])
  
    png(filename=file.path(YOUR.STORAGE.PATH,STORE.SUBDIR,savefilename), 
      height=png.width,
      width=png.width,
      units="px"
  )
...
# 
# 
# png(filename=file.path(YOUR.STORAGE.PATH,STORE.SUBDIR,"tempA.png"), 
#     height=10000,
#     width=10000,
#     units="px"
# )
