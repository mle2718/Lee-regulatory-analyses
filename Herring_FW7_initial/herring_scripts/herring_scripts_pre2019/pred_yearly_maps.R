...
mypat1<-glob2rx("81*.tif")
mypat2<-glob2rx("269*.tif")
...
#   #To fix this, we will set all raster values greater than the upper bound to be slightly less than this upper bound
#   
#   ub=mybreaks_class2[length(mybreaks_class2)]
#   myras[myras>=ub]<-floor(ub)
#   
...
#   subsamp<-subsamp[subsamp>jenks.lowerbound]
#   mylist3[[length(mylist3)+1]] = subsamp
# }
# 
# 
# myvals<-unlist(mylist3)
# glob_max<-max(myvals)
# mysubs<-sample(myvals,num_jenks_subs)
# 
# 
# 
# myclass <- classIntervals(mysubs, n=nbreaks,style="jenks")
# mybreaks_class3<-c(0,myclass$brks)
# #   #End JENKS BREAKS code
# 
# 
# 
...
#   #To fix this, we will set all raster values greater than the upper bound to be slightly less than this upper bound
#   
#   ub=mybreaks_class3[length(mybreaks_class3)]
#   myras[myras>=ub]<-floor(ub)
#   
...
#   jconsfilename=paste(jconsfilename,"png",sep=".")
#   title_opts3<-list(parsed_list3$NAME[i],cex=2)
#   
#   
#   p<-levelplot(myras, margin=FALSE, main=title_opts3, par.setting=mycoloropts,scales=myscaleopts)
#   
#   jcons<-levelplot(myras, margin=FALSE, main=title_opts3, 
#                    par.setting=mycoloropts,scales=myscaleopts, at=mybreaks_class3, 
#                    xlim=my.xlimit, ylim=my.ylimit, colorkey=myckey)
#   
#   
#   
...
#   p<-levelplot(myras, margin=FALSE, main=parsed_list3$NAME[i], par.setting=mydichrome,scales=list(draw=FALSE))
