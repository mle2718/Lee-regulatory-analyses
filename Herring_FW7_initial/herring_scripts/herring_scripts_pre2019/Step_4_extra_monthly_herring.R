  ...
  exportfolder = file.path(ML.PROJECT.PATH,paste0(todaysdate, "_task12_rasterexport"))
...
rm(list="FILE_INFO")
  ...
  FINAL$MY_MARGIN<-paste(FINAL$NESPP3,"QTYKEPT",FINAL$MONTH, sep="_")
...
if(NROW(fileswanted)==1) {
  ...
  fileswanted = c(fileswanted, paste0(file.path(SSBdrive,"Data","BASERASTER_AEA_2.grd"),"@BLANK@"))
  ...
  RAS1 = sfLapply(fileswanted,function(x,...) {     #usually run in sfLapply
    ...
    HOL2 = calc(HOL2,fun=function(z) {z*0})
    #target.sum = TARGET
