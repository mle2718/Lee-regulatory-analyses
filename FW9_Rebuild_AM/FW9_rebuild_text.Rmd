...
Table \ref{RFA_all_revenue_changes_all} illustrates the expected changes in receipts. Most of the additional revenue accrues to small firms; however, the increase in gross receipts from fishing is quite small in magnitude.  Figure \ref{fig:Gross_Rec_Active}} is histogram of the change in gross receipts for the small firms that are active in the fishery[^fn_inactive]. There are only nine large firms, therefore a histogram of the changes in their revenues is not useful. Note that the large firms are projected to have an additional $26,000 in combined revenue. 
...
\begin{table}[htbp]
  ...
  \end{scriptsize}
    \caption{Expected Changes in Receipts (Millions of nominal USD) \label{RFA_all_revenue_changes_all}}
  \end{center}
\end{table}


```{r Gross_Rec_Active, fig.show = "hold", out.width = "48%", fig.cap="Changes in Gross Receipts for the Small Active Firms",  fig.align = "center", echo=FALSE}
knitr::include_graphics(file.path(my_images,paste0("RFA_active_change_in_gross_receipts.png")))
```
 
 Figure \ref{fig:Gross_Rec_Active_Pct} is histogram of the percentage change in gross receipts for the small firms that are active in the fishery. Most firms will experience an increase of less than 0.5\% of gross receipts.
 
 
```{r Gross_Rec_Active_Pct, fig.show = "hold", out.width = "48%", fig.cap="Changes in Gross Receipts in Percentage terms for the small Active Firms",  fig.align = "center", echo=FALSE}
knitr::include_graphics(file.path(my_images,paste0("RFA_active_pct_change_in_gross_receipts.png")))
```
