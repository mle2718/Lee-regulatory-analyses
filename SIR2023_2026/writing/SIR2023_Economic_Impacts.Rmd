...
See Section 3.0 for a description of the fishery

### Statement of the Problem 

### Economic impacts relative to the baseline
The primary change that would affect small firms is a change in the Annual Catch limit from 4,098mt in 2023 to 12,429mt, 19,189mt, and 23,961mt in 2023-2026 respectively.  Recent catches have been close to the ACLs, so we assume that catch is equal to the ACLs in the future. other changes are expected to have a minimal economic effects.
...
Year<-c("Status Quo", "2023", "2024", "2025")
...
assign("baselineRevenue",proj_rev[proj_rev$Year =="Status Quo" ,"Revenue"])
...
The directly regulated entities are the firms that currently hold at least 1 Northeast US herring fishing permit (Categories A, B, C, D, or E).  Table \ref{tab:Directly_Regulated_Entities} describes numbers of directly regulated entities, their main activities, and their revenues from various sources.   729 small firms derive the majority of their revenue from commercial fishing operations.   Eleven (11) of the large firms derive the majority of their revenue from commercial fishing activities.  There are 121 small firms that derive a majority of their revenue from for-hire recreational fishing activities. The for-hire firms, while they held at least one herring permit, did not derive any revenue from herring.
...
kbl(ABCEherring2021_table, digits=0,booktabs=T, align=c("l",rep('r',times=7)), caption =  "Number and Characterization of the Active Direcly Regulated Entities with A, B, C or E permits, Average Trailing Three Years of Revenue") %>%
    #column_spec(5:8, width = "2cm")
    kable_styling(full_width = T) %>% 
      row_spec(0,bold=TRUE) 

```
Many of the firms described in tables \ref{tab:Directly_Regulated_Entities} and \ref{ABCEherring2021_table} are not actively engaged in the herring fishery.  The herring fishery has had historically low ACLs  since 2018 and some firms have ceased their participation in the fishery. They may hold herring permits to preserve the option to fish. 

Table \ref{tab:Active_DREs} describes numbers of active directly regulated entities that hold a category A, B, C, or E herring permit, their main activities, and their revenues from various sources. Because there are fewer than 3 large firms, we only present a description of the small firms.
...
Figures  \ref{fig:boxplotsR} and \ref{fig:boxplots_H} illustrate the projected yearly distribution of total and herring revenues from the Active vessels with A,B,C, or E herring permits.

```{r boxplotsR, fig.cap=" Projected Firm Level Revenue, Small firms only"}
...
```{r boxplots_H, fig.cap="Projected Firm Level Herring Revenue, Small firms only"}
