...
```	
 ...
 <!---- 
 The data_process_SIR2023.Rmd knits a child RMD.  There are a bunch of 'options' that were originally set in these chunks. I've moved most of them to the global_options and set_switches sections. but not all of them. And some are hard coded. 
...
Benefits: The proposed action will set Annual Catch limits and other fishery specification for 2023-2026. In aggregate these changes will allow the fishing industry to catch, land, and sell more herring. Higher revenues are expected. 

Costs: Because ACLs are higher than the status quo, it is reasonable to expect that stock levels in the future will be slightly lower. Rebuilding may take slightly longer. Furthermore, the stock itself has value: it produces future generations of fish and higher stock levels make harvesting less costly. 

A regulatory change is the New Brunswick weir adjustment regs at 648.201(h), where the threshold decreases from 3,012 mt to 2,722 mt.  If New Brunswick weir catch is under 2,722 NMFS will increase the ACL by 1,000mt.  The lower threshold imposes a slightly cost because it is less likely that NMFS will increase the ACL by 1,000mt. 
...
See Section N.N for a description of the fishery
...
Framework 8 contains an ACL for 2023. This ACL is 4,098mt and is a reasonable baseline against which to evalute the economic effects of the propsed action.  Under the baseline, we project prices of **\$791/metric ton** and gross revenues of **\$3.24M**.
...
```{r npv }
# Compute NPV of change in revenue at the 3% and 7% discount rates.
proj_rev$df3<-1.03
proj_rev$df7<-1.07
...
The directly regulated entities are the firms that currently hold at least 1 Northeast US herring fishing permit (Categories A, B, C, D, or E).  Table \ref{tab:make_DRE_table} describes numbers of directly regulated entities, their main activities, and their revenues from various sources.   `r  small_fishing_firms` small firms derive the majority of their revenue from commercial fishing operations.   `r large_fishing_firms` of the large firms derive the majority of their revenue from commercial fishing activities.  There are   `r small_forhire_firms` small firms that derive a majority of their revenue from for-hire recreational fishing activities. The for-hire firms, while they held at least one herring permit, did not derive any revenue from herring.
...
While many of the firms described in table \ref{tab:make_DRE_table} hold a herring permit, some firms only hold a category-D open access permit which has a 6,600lb possession limit.  These firms have limited opportunity to increase their catch of herring when catch limits increase. They are also less impacted by closures of the fishery when the catch limits are reached, because the possession limits are set to 2,000 pounds when this occurs. Many of the firms described in tables \ref{tab:make_DRE_table} are not actively engaged in the herring fishery.  The herring fishery has had historically low ACLs  since 2018 and some firms have stopped participating in the fishery. They may hold herring permits to preserve the option to fish. The increases ACLs in the proposed action may or may not be high enough to induce participation.

Table \ref{tab:Active_DREs} describes numbers of active directly regulated entities that hold a category A, B, C, or E herring permit, their main activities, and their revenues from various sources. Because there are fewer than 3 large firms, we only present a description of the small firms.  The small firms identified in table \ref{tab:Active_DREs} are the firms most likely to be impacted by the increases in ACLs in the proposed action.

<!---Table \ref{tab:ABCE_DREs} describes numbers of directly regulated entities that hold a category A, B, C, or E herring permit, their main activities, and their revenues from various sources. 
--->

```{r ABCE_DREs, eval=FALSE}
# Make a keyfile based on herring2021 that just has firms with at least 1 LA herring permit. 

herringABCE2021<-herring2021 %>%
  filter(herringABCE>=1)
herringABCE_DREs<-left_join(herringABCE2021,summary_affiliates,by='affiliate_id')
...
ABCEherring2021_table <- herringABCE_DREs %>%
  filter(Type !="NO_REV") %>%
  group_by(Size, Type) %>%
  summarise(Firms=n(),
            Vessels=sum(vessels),
...
kbl(ABCEherring2021_table, digits=0,booktabs=T, align=c("l",rep('r',times=7)), caption =  "Number and Characterization of the Direcly Regulated Entities with A, B, C or E permits, Average Trailing Three Years of Revenue") %>%
    #column_spec(5:8, width = "2cm")
    kable_styling(full_width = T) %>% 
      row_spec(0,bold=TRUE) 
...
## Projected Revenues for Small Firms
To describe the effects of the changes in catch limits on small firms, we project firm-level revenue corresponding to 2023-2026. We assume that the share of herring landings for each firm is equal to their 2019-2021 average. We also assume the firms non-herring revenues are constant. Inactive firms drop out and we focus on the vessels that have ABCE permits.  
<!---
This is going to be based on the Active_ABCE data table.
...
# Compute the firm level averages and shares.
deflated_affiliates<-affiliates %>%
   ...
   group_by(affiliate_id,year) %>%
  summarise(across(value_permit:value168, sum)) %>%  # Creates a firm-year dataset of values 
    ungroup() %>%
  group_by(affiliate_id) %>%
  summarise(across(value_permit:value168, mean)) %>% # Takes the mean of the firm-level dataset of values
    rename(value_firm=value_permit) %>%
  ungroup() %>%
  mutate(th=sum(value168)) %>%
  mutate(herring_share=value168/th,
         value_other=value_firm-value168) %>%
...
projected_firm_rev_ABCE_table <- projected_firm_rev_ABCE %>%
  ...
  filter(Type !="NO_REV" & Size=="Small") %>%
  summarise(Firms=n(),
            "Baseline Gross Receipts"=round(mean(value_firm),0),
            "Projected Gross Receipts"=round(mean(sim_value_firm),0),
            "Baseline Herring Receipts"=round(mean(value168),0),
            "Projected Herring Receipts"=round(mean(sim_value168),0)) %>%
  mutate(across(c(`Firms`), ~ number(.x, big.mark=","))) %>%
  mutate(across(c(`Baseline Gross Receipts`,`Projected Gross Receipts`, `Baseline Herring Receipts`, `Projected Herring Receipts` ), ~ number(.x, accuracy=1000,prefix="$", big.mark=","))) %>%
...
kbl(projected_firm_rev_ABCE_table, digits=0,booktabs=T, align=c("l",rep('r',times=7)), caption =  "Projected and baseline gross reciepts and herring receipts for Small Firms. Figures for large firms cannot be show due to data confidentiality.") %>%
    #column_spec(5:8, width = "2cm")
    kable_styling(full_width = T) %>% 
      row_spec(0,bold=TRUE) 
...
Figures  \ref{figure_boxR} and \ref{figure_boxH} illustrate the projected yearly distribution of total and herring revenues from the Active vessels with A,B,C, or E herring permits.
